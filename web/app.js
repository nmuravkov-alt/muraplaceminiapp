...
function initApp() {
  ...
  if (tg) {
    tg.ready();
    tg.expand?.(); // важно для Direct Link/Main App
  }
  ...
  // Если по какой-то причине DOM-элементы не найдены — не падаем в белый экран
  if (!heroEl || !categoriesEl || !productsEl || !cartBtn || !cartCount || !writeBtn || !checkoutBtn || !sheet || !backdrop) {
    console.warn("Some DOM elements are missing. Check your HTML ids.");
  }
